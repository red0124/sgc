...
#define _SGC_INIT_PRIVATE_FORWARD_LIST_FUNCTION_DECLARATIONS(T, N)             \
    static struct _m_##N##_node* _m_##N##_prev(struct _m_##N##_node* head,     \
...
#define _SGC_INIT_UNIQUE_FORWARD_LIST_FUNCTIONS(T, N)                          \
    ...
    static struct _m_##N##_node* _m_##N##_prev(struct _m_##N##_node* head,     \
                                               struct _m_##N##_node* n) {      \
        struct _m_##N##_node* curr = (head == n) ? NULL : head;                \
        struct _m_##N##_node* next = curr;                                     \
        while (next && next != n) {                                            \
            curr = next;                                                       \
            next = next->next_;                                                \
        }                                                                      \
    ...
    void N##_pop_back(struct N* l) {                                           \
        if (l->size_) {                                                        \
            ...
            l->tail_ = _m_##N##_prev(l->head_, l->tail_);                      \
            if (!l->shared_) {                                                 \
                T##_free(&tmp->data_);                                         \
            }                                                                  \
    ...
    void N##_iterator_cend(const struct N* const l, struct N##_iterator* i) {  \
        ...
        i->head_ = l->head_;                                                   \
                                                                               ...
                                                                               \
    void N##_iterator_prev(struct N##_iterator* i) {                           \
        i->curr_ = _m_##N##_prev(i->head_, i->curr_);                          \
    }

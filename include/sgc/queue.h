...
#define SGC_INIT_QUEUE(T, N)                                                   \
    ...
    static void N##_resize(struct N* q) {                                      \
        if (q->_size == q->_max) {                                             \
            ...
            q->_max = (q->_max == 0) ? 1 : q->_max * 2;                        \
                                                                               \
            q->_data = (T*)sgc_realloc(q->_data, sizeof(T) * q->_max);         \
                                                                               \
            if (q->_front > q->_back) {                                        \
                size_t first_part = q->_back;                                  \
                size_t second_part = max - q->_front;                          \
                if (first_part > second_part) {                                \
                    memcpy(q->_data + (q->_max - second_part),                 \
                           q->_data + q->_front, second_part * sizeof(T));     \
    ...
    void N##_set_front(struct N* q, T new_el) {                                \
        if (q->_size) {                                                        \
            SGC_REPLACE(T##_copy, T##_free, q->_data[q->_front], new_el,       \
                        q->_shared);                                           \
        }                                                                      \
    }                                                                          \
                                                                               \
    ...
    void N##_set_back(struct N* q, T new_el) {                                 \
        if (q->_size) {                                                        \
            SGC_REPLACE(T##_copy, T##_free, q->_data[q->_back], new_el,        \
                        q->_shared);                                           \
        }                                                                      \
    }                                                                          \
                                                                               \

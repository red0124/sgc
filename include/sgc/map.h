...
#define _SGC_INIT_PRIVATE_MAP_FUNCTION_DECLARATIONS(K, V, N)                   \
    ...
    static struct _m_##N##_node* _m_##N##_node_new(const K* const k,           \
                                                   const V* const v,           \
                                                   int is_shared_key,          \
                                                   int is_shared);             \
    ...
    static void _m_##N##_rotate(struct N* m, struct _m_##N##_node* n,          \
                                struct _m_##N##_node* parent,                  \
                                struct _m_##N##_node* gparent);                \
    static void _m_##N##_correct_tree(struct N* m, struct _m_##N##_node* n,    \
                                      struct _m_##N##_node* p,                 \
                                      struct _m_##N##_node* gp);               \
    static void _m_##N##_check_color(struct N* m, struct _m_##N##_node* n);    \
    static void _m_##N##_node_insert(struct N* m, K* k, V* v);                 \
    static V* _m_##N##_node_insert_or_get(struct N* m, K* k);                  \
    static struct _m_##N##_node* _m_##N##_node_find(struct N* m, K k);         \
...
#define _SGC_INIT_UNIQUE_MAP_FUNCTIONS(K, V, N)                                \
    static struct _m_##N##_node* _m_##N##_node_new(const K* const k,           \
                                                   ...
                                                   int is_shared_key,          \
                                                   int is_shared) {            \
        struct _m_##N##_node* n = sgc_malloc(sizeof(struct _m_##N##_node));    \
                                                                               \
    ...
    void N##_set_share_key(N* m, int is_shared) {                              \
        m->shared_key_ = is_shared;                                            \
                                                                               ...
                                                                               \
    static void _m_##N##_node_insert(struct N* m, K* k, V* v) {                \
        struct _m_##N##_node* parent = m->root_;                               \
    ...
    V* N##_at(struct N* m, K k) {                                              \
        ...
        if (m->root_ == SGC_MAP_LEAF) {                                        \
            ...
            ret = _m_##N##_node_insert_or_get(m, &k);                          \
            m->root_->color_ = SGC_MAP_BLACK;                                  \
        }                                                                      \
        return ret;                                                            \
    }                                                                          \
